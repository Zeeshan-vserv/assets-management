...
function StoreLocation() {
  ...
  const [editStoreLocation, setEditStoreLocation] = useState(null);
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        Cell: ({ row }) => (
          <IconButton
            onClick={() => handleUpdateStoreLocation(row.original._id)}
            color="primary"
            aria-label="edit"
          >
            <MdModeEdit />
      ...
      },
      {
        ...
        Cell: ({ row }) => (
          <IconButton
            onClick={() => handleDeleteStoreLocation(row.original._id)}
            color="error"
            aria-label="delete"
          >
            <DeleteIcon />
  ...
  const updateStoreLocationChangeHandler = (e) => {
    const { name, value } = e.target;
  ...
  const table = useMaterialReactTable({
    ...
    renderTopToolbarCustomActions: ({ table }) => {
      return (
        <Box>
          <Button
            onClick={() => setAddStoreLocationModal(true)}
            variant="contained"
            size="small"
