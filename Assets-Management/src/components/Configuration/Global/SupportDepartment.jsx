...
//   filename: "Assets-Management-SupportDepartment.csv",
// });
...
//         header: "Support Department Department Id",
//       },
//       {
...
//         header: "Support Department Head",
//       },
//       {
//         id: "edit",
//         header: "Edit",
...
const SupportDepartment = () => {
  ...
  const fetchDepartment = async () => {
    try {
      ...
      toast.error("Error fetching support departments");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const columns = useMemo(
    () => [
      {
        accessorKey: "supportDepartmentId",
        header: "Support Department Id",
      },
      {
  ...
  const handleUpdateDepartment = (id) => {
    ...
    if (departmentToEdit) {
      setEditDepartment({
        _id: departmentToEdit._id,
        supportDepartmentName: departmentToEdit.supportDepartmentName,
      });
      setOpenUpdateModal(true);
    }
  };
  ...
  const updateNewDepartmentHandler = async (e) => {
    ...
    try {
      const formData = {
        supportDepartmentName: editDepartment.supportDepartmentName,
      };
      await updateSupportDepartment(editDepartment._id, formData);
      await fetchDepartment();
      setOpenUpdateModal(false);
  ...
  const deleteDepartmentConfirmationHandler = async (e) => {
    e.preventDefault();
    try {
      await deleteSupportDepartment(deleteDepartmentId);
      toast.success("Support Department Deleted successfully");
      await fetchDepartment();
    } catch (error) {
      toast.error("Error deleting support department");
    }
    setDeleteConfirmationModal(false);
    setDeleteDepartmentId(null);
  };
  ...
  const addNewDepartmentHandler = async (e) => {
    ...
    const formData = {
      userId: user?.userId,
      supportDepartmentName: addNewDepartment.supportDepartmentName,
    };
    const response = await createSupportDepartment(formData);
    if (response?.data?.success) {
      toast.success("Support Department Added successfully");
      await fetchDepartment();
      setNewDepartment({
        supportDepartmentName: "",
      });
  ...
  const handlePdfData = () => {
    ...
    doc.save("Assets-Management-SupportDepartment.pdf");
  };
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-2">
                    ...
                    <TextField
                      ...
                      required
                      fullWidth
                      value={addNewDepartment?.supportDepartmentName || ""}
                      onChange={addNewDepartmentChangeHandler}
                      placeholder="Enter Department Name"
                      variant="standard"
                      sx={{ width: 250 }}
              ...
              <form onSubmit={updateNewDepartmentHandler} className="space-y-4">
                <div className="flex items-center gap-2">
                  ...
                  <TextField
                    ...
                    required
                    fullWidth
                    value={editDepartment?.supportDepartmentName || ""}
                    onChange={addNewDepartmentInputChangeHandler}
                    placeholder="Enter Department Name"
                    variant="standard"
                    sx={{ width: 250 }}
